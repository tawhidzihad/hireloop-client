...
export default function PostJobForm({ company }) {
	...
	const onSubmit = async (data) => {
		if (company.status !== "Approved") {
			toast.error("Your company must be approved before posting jobs.");
			return;
		}
			...
			<div className="overflow-hidden rounded-3xl border border-white/10 bg-zinc-950">
				...
				>
					...
					<section>
						...
						<div className=" flex flex-col gap-3 rounded-2xl border border-white/10 bg-zinc-900/50 p-4 backdrop-blur-xl sm:flex-row sm:items-center sm:justify-between">
							<div className="flex items-center gap-3">
								<div className="relative flex h-10 w-10 items-center justify-center overflow-hidden rounded-xl border border-white/10 bg-white/5">
									{company.logo ? (
										<Image
											src={company.logo}
											alt={company.name}
							...
							<span
								className={`w-fit rounded-full border border-green-500/20 bg-green-500/10 px-3 py-1.5 text-sm font-medium ${statusStyles[company.status]}`}
							>
								{company.status}
							</span>
						</div>
					...
					<div className="flex flex-col-reverse gap-3 border-t border-white/10 pt-8 sm:flex-row sm:justify-end">
						...
						<button
							type="submit"
							disabled={isSubmitting || company?.status !== "Approved"}
							className=" rounded-xl bg-[#6D5DFD] px-5 py-3 font-medium text-white transition-all duration-200 hover:bg-[#7d70ff] disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:bg-[#6D5DFD]"
						>
							{isSubmitting
								? "Publishing..."
								: company?.status !== "Approved"
